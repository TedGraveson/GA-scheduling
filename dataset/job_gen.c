...
#define FIRMACIND 1  /* 0,1: first machine index           */ 
...
#endif
{
  ...
  sprintf(name,"ta%03d.txt", p);                 /* file name construction */ 
  ...
  for(j = 0; j < S[p].num_jobs; ++j) {
    for(i = 0; i < S[p].num_mach; ++i) {
      // fprintf(f,"%2d %2d ", i+FIRMACIND, d[i][j]);
      if(i != S[p].num_mach-1) {
        fprintf(f,"%d ", d[i][j]); /* write machine and job */ 
      }
      else {
        fprintf(f,"%d", d[i][j]); // No space on last job
      }
